...
| --- | --- | --- |
| 3.7.0 | [subnet-calculator-3.7.0.tar.gz](releases/subnet-calculator-3.7.0.tar.gz) | **UX polish + layout refactor (2026-05-09).** IPv6 tools grouped into 5 collapsible categories with localStorage persistence. Mobile h1 wrap + 2x2 tab grid below 480px. `templates/layout.php` extracted into 35 `_tools/*.php` partials (3387 → ~948 lines). `:focus` → `:focus-visible` sweep eliminates sticky outlines after mouse clicks. Per-drawer a11y polish (tab order, focus return, reduced motion). (#420) `CACHE_NAME` → `sc-v3.7.0` |
