...
**Polish patch.** Drops the value-prop chip strip added in v3.9.0 (#446).
On operator review the chips read as filler — the audience already knows
what the calculator does, and the real UX win from #446 was the
top-of-fold card anchoring (which stays). The chips were `aria-hidden`
and mobile-hidden anyway. Card now sits cleanly with empty space above.

### Removed

- `.value-prop` + `.value-prop-chip` strip above the card. Anchoring
  behavior from v3.9.0 is unchanged.
